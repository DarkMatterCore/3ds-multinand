...
#define N3DS_SAMSUNG_NAND_1		0x4D800000									// 1240 MiB
#define N3DS_SAMSUNG_NAND_2		0x74800000									// 1864 MiB
...
#define N3DS_MINIMUM_FAT		round4MB(SECTOR_SIZE + N3DS_SAMSUNG_NAND_1)	// FILE_BEGIN + 512 bytes + 1240 MiB + 4 MB alignment
...
#define GET_ARRAYSIZE(x)		((sizeof((x))) / (sizeof((x)[0])))			// Returns the number of elements in an array
#define NAND_NUM_STR(x)			((x) == 1 ? L"st" : ((x) == 2 ? L"nd" : ((x) == 3 ? L"rd" : L"th")))
#define NAND_TYPE_STR(x)		(((x) == O3DS_TOSHIBA_NAND || (x) == (O3DS_TOSHIBA_NAND + SECTOR_SIZE) || (x) == (N3DS_TOSHIBA_NAND) || (x) == (N3DS_TOSHIBA_NAND + SECTOR_SIZE)) ? L"Toshiba" : (((x) == O3DS_SAMSUNG_NAND || (x) == (O3DS_SAMSUNG_NAND + SECTOR_SIZE) || (x) == N3DS_SAMSUNG_NAND_1 || (x) == (N3DS_SAMSUNG_NAND_1 + SECTOR_SIZE) || (x) == N3DS_SAMSUNG_NAND_2 || (x) == (N3DS_SAMSUNG_NAND_2 + SECTOR_SIZE)) ? L"Samsung" : L"**Unknown**"))
#define FAT_LAYOUT_STR(x)		(((x) == O3DS_LEGACY_FAT || (x) == N3DS_LEGACY_FAT) ? L"Legacy" : (((x) == O3DS_MINIMUM_FAT || (x) == N3DS_MINIMUM_FAT) ? L"Minimum" : L"Default"))
...
typedef struct {
	...
	int64_t emunand_offsets[MAX_NAND_NUM];
